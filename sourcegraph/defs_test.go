...
func TestDefsService_ListImplementations(t *testing.T) {
	setup()
	defer teardown()
	...
	var called bool
	mux.HandleFunc(urlPath(t, router.DefImplementations, map[string]string{"RepoURI": "r.com/x", "UnitType": "t", "Unit": "u", "Path": "p"}), func(w http.ResponseWriter, r *http.Request) {
		called = true
		testMethod(t, r, "GET")
	...
	implementations, _, err := client.Defs.ListImplementations(DefSpec{Repo: "r.com/x", UnitType: "t", Unit: "u", Path: "p"}, nil)
	if err != nil {
		t.Errorf("Defs.ListImplementations returned error: %v", err)
	}
	...
	if !reflect.DeepEqual(implementations, want) {
		t.Errorf("Defs.ListImplementations returned %+v, want %+v", implementations, want)
	}
}

func TestDefsService_ListInterfaces(t *testing.T) {
	setup()
	defer teardown()
	...
	var called bool
	mux.HandleFunc(urlPath(t, router.DefInterfaces, map[string]string{"RepoURI": "r.com/x", "UnitType": "t", "Unit": "u", "Path": "p"}), func(w http.ResponseWriter, r *http.Request) {
		called = true
		testMethod(t, r, "GET")
	...
	interfaces, _, err := client.Defs.ListInterfaces(DefSpec{Repo: "r.com/x", UnitType: "t", Unit: "u", Path: "p"}, nil)
	if err != nil {
		t.Errorf("Defs.ListInterfaces returned error: %v", err)
	}
	...
	if !reflect.DeepEqual(interfaces, want) {
		t.Errorf("Defs.ListInterfaces returned %+v, want %+v", interfaces, want)
	}
}
